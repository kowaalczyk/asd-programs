...
typedef int id_t; // TODO: wtf is wrong here?
...
typedef pair<cost_t, id_t> connection_dijkstra_t; /// := {connection_temporary_cost, connection_id} for use in Dijkstra queue
...
id_t get_destination(const connection_t &connection) {
...
int main() {
    ...
    while(!dijkstra_q.empty()) {
        // TODO: Make sure break condition is optimal (spanning tree for higher k's might contain different nodes => if complexity is too high, add decrease_key)
        ...
        for(id_t following_connection_id : connections_from[current_node_data.second]) {
            connection_t &following_connection = connections[following_connection_id];
